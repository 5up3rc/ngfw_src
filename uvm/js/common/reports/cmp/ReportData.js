    ...
    controller: {
        ...
        onCellClick: function (cell, td, cellIndex, record, tr, rowIndex) {
            ...
            if (cellIndex === 0) {
                ...
                var msg = 'Add <strong>' + TableConfig.getColumnHumanReadableName(column) + '</strong> column to the Global Conditions?'.t(),
                    action = 'add';
                if (me.getViewModel().get('disablecConds')[column]) {
                    msg = 'The <strong>' + TableConfig.getColumnHumanReadableName(column) + '</strong> column is already in Global Conditions!<br/> Replace its value?';
                    action = 'replace';
                }


                Ext.Msg.show({
                    title: (action === 'add' ? 'Add' : 'Replace') + ' ' + 'Global Condition',
                    message: msg + '<br/><br/>' +
                             '<strong>' + TableConfig.getColumnHumanReadableName(column) + ' [' + column + '] = ' + value + '</strong>',
                    buttons: Ext.Msg.YESNO,
                    icon: Ext.Msg.QUESTION,
                    fn: function (btn) {
                        if (btn === 'yes') {
                            grid.up('#reports').getController().conditionFromData(column, value, action);
                        }
                    }
                });
